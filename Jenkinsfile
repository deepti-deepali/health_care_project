  ...
  stages {
    stage('Git Checkout') {
      steps {
        ...
        git branch: 'main', url: 'https://github.com/deepti-deepali/health_care_project.git'
                        }
            }
    ...
    stage('Generate Test Report') {
     ...
     stage('Create Docker Image') {
      steps {
        ...
        sh 'docker build -t deeptideepali/healthcare:1.0 .'
                          }
            }
     stage('Login to Dockerhub') {
      steps {
        echo 'This stage will loginto Dockerhub' 
        withCredentials([usernamePassword(credentialsId: 'dockernewcred', passwordVariable: 'dockerpass', usernameVariable: 'dockeruser')]) {
